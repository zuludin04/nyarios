...
class ChatRepository {
  ...
  const ChatRepository({required this.localSource, required this.remoteSource});
  ...
  Stream<List<Message>> streamChatMessages(String? chatId) {
    return remoteSource.streamMessages(chatId);
  }

  Future<void> deleteMessages(String chatId, List<Message> messages) async {
    await remoteSource.messagesBatchDelete(chatId, messages);
  }
  ...
  Stream<List<RecentChat>> streamRecentChats() async* {
    final user = await localSource.getUserProfile();
    yield* remoteSource.streamRecentChats(user.userId);
  }
}
